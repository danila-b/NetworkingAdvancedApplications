...
function getPopularProductRange() {
  let segment = 0
  // exec.scenario.progress gives us a value between 0-1
  if (exec.scenario.progress < 0.25) {
    segment = 0
  } else if (exec.scenario.progress < 0.5) {
    segment = 1
  } else if (exec.scenario.progress < 0.75) {
    segment = 2
  } else {
    segment = 3
  }

  const rangeStart = (segment * numberOfPopularProducts) + 1;
  const rangeEnd = (segment + 1) * numberOfPopularProducts;
...
export default function () {
  ...
  let currentPopularRange = getPopularProductRange();
  let productId;
  if (usePopularProduct) {
    // Select a random product from the high-traffic list
    productId = Math.floor(Math.random() * (currentPopularRange.max - currentPopularRange.min + 1)) + currentPopularRange.min;
  } else {
  ...
  if (productId >= currentPopularRange.min && productId <= currentPopularRange.max) {
